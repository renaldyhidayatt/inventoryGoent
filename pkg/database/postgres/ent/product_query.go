...
type ProductQuery struct {
	...
	withProductkeluar *ProductKeluarQuery
...
// QueryProductkeluar chains the current query on the "productkeluar" edge.
func (pq *ProductQuery) QueryProductkeluar() *ProductKeluarQuery {
	query := (&ProductKeluarClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryProductmasuk() *ProductMasukQuery {
	query := (&ProductMasukClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withProductkeluar: pq.withProductkeluar.Clone(),
		withProductmasuk:  pq.withProductmasuk.Clone(),
...
// WithProductkeluar tells the query-builder to eager-load the nodes that are connected to
// the "productkeluar" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProductQuery) WithProductkeluar(opts ...func(*ProductKeluarQuery)) *ProductQuery {
	query := (&ProductKeluarClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProductkeluar = query
	return pq
}
...
func (pq *ProductQuery) WithProductmasuk(opts ...func(*ProductMasukQuery)) *ProductQuery {
	query := (&ProductMasukClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withProductmasuk = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	var (
		...
		loadedTypes = [3]bool{
			pq.withCategory != nil,
			pq.withProductkeluar != nil,
			pq.withProductmasuk != nil,
		}
	)
	...
	if query := pq.withProductkeluar; query != nil {
		if err := pq.loadProductkeluar(ctx, query, nodes,
			func(n *Product) { n.Edges.Productkeluar = []*ProductKeluar{} },
			func(n *Product, e *ProductKeluar) { n.Edges.Productkeluar = append(n.Edges.Productkeluar, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withProductmasuk; query != nil {
		if err := pq.loadProductmasuk(ctx, query, nodes,
			func(n *Product) { n.Edges.Productmasuk = []*ProductMasuk{} },
			func(n *Product, e *ProductMasuk) { n.Edges.Productmasuk = append(n.Edges.Productmasuk, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadProductkeluar(ctx context.Context, query *ProductKeluarQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductKeluar)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.ProductkeluarTable)
		s.Join(joinT).On(s.C(productkeluar.FieldID), joinT.C(product.ProductkeluarPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(product.ProductkeluarPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.ProductkeluarPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*ProductKeluar](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "productkeluar" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (pq *ProductQuery) loadProductmasuk(ctx context.Context, query *ProductMasukQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductMasuk)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(product.ProductmasukTable)
		s.Join(joinT).On(s.C(productmasuk.FieldID), joinT.C(product.ProductmasukPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(product.ProductmasukPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.ProductmasukPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
