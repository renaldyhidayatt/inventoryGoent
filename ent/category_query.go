...
func (cq *CategoryQuery) QueryProductkeluar() *ProductKeluarQuery {
	query := (&ProductKeluarClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withProductkeluar: cq.withProductkeluar.Clone(),
...
func (cq *CategoryQuery) WithProductkeluar(opts ...func(*ProductKeluarQuery)) *CategoryQuery {
	query := (&ProductKeluarClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withProductkeluar = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withProductkeluar; query != nil {
		if err := cq.loadProductkeluar(ctx, query, nodes,
			func(n *Category) { n.Edges.Productkeluar = []*ProductKeluar{} },
			func(n *Category, e *ProductKeluar) { n.Edges.Productkeluar = append(n.Edges.Productkeluar, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Category, init func(*Category), assign func(*Category, *Product)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(category.ProductsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.ProductsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ProductsPrimaryKey[1]))
...
func (cq *CategoryQuery) loadProductkeluar(ctx context.Context, query *ProductKeluarQuery, nodes []*Category, init func(*Category), assign func(*Category, *ProductKeluar)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Category)
	nids := make(map[int]map[*Category]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ProductkeluarTable)
		s.Join(joinT).On(s.C(productkeluar.FieldID), joinT.C(category.ProductkeluarPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(category.ProductkeluarPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ProductkeluarPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
