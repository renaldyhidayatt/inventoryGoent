...
func NewProductMasukRoutes(prefix string, db *ent.Client, router *chi.Mux, context context.Context) {
	repository := repository.NewProductMasukRepository(db, context)
	service := services.NewProductMasukService(repository)
	handler := handler.NewProductMasukHandler(service)
