...
func NewSupplierRoutes(prefix string, db *ent.Client, router *chi.Mux, context context.Context) {
	repository := repository.NewSupplierRepository(db, context)
	service := services.NewSupplierService(repository)
	handler := handler.NewSupplierService(service)
